									...
									uint32_t tmp_vir_addr)
{
	...
	else if(srcFormat == HAL_PIXEL_FORMAT_YCrCb_420_SP) {
		input_format = HW_SCALE_DATA_YUV420;
	}
	else if(srcFormat == HAL_PIXEL_FORMAT_YV12) {
		input_format = HW_SCALE_DATA_YUV420_3FRAME;
	}
	else {
		return -1;
	}
	if(dstFormat == HAL_PIXEL_FORMAT_RGBX_8888)
	if(dstFormat ==  HAL_PIXEL_FORMAT_YCbCr_420_SP) {
